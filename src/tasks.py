...
--settings={0}".format(_localsettings()), pty=True)
    ctx.run("python manage.py loaddata /usr/src/infomapnode/fixtures/geonode_themes.json \
--settings={0}".format(_localsettings()), pty=True)
    ctx.run("python manage.py loaddata /usr/src/infomapnode/fixtures/initial_data.json \
--settings={0}".format(_localsettings()), pty=True)
...
def _docker_host_ip():
    try:
        ...
        ip_list = client.containers.run(
            BOOTSTRAP_IMAGE_CHEIP,
            network_mode='host').split("\n")
    except Exception:
        import traceback
        traceback.print_exc()
        ip_list = ['127.0.0.1', ]
    if len(ip_list) > 1:
        print(f"Docker daemon is running on more than one \
address {ip_list}")
        print(f"Only the first address:{ip_list[0]} will be returned!")
    else:
        print(f"Docker daemon is running at the following \
address {ip_list[0]}")
    return ip_list[0]
...
def _localsettings():
    settings = os.getenv('DJANGO_SETTINGS_MODULE', 'infomapnode.settings')
    return settings
